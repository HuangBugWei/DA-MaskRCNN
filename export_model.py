...
if __name__ == "__main__":
    ...
    lazycfg = LazyConfig.load(args.config_file)
    # torch_model = build_model(cfg)
    lazycfg.model.backbone.bottom_up.stem.norm = \
    lazycfg.model.backbone.bottom_up.stages.norm = "BN"
    lazycfg.model.backbone.norm = "BN"
    torch_model = instantiate(lazycfg.model)
    torch_model.backbone.bottom_up._out_features = \
        list(torch_model.backbone.bottom_up._out_features)
    torch_model.proposal_generator.in_features = \
        list(torch_model.proposal_generator.in_features)
    torch_model.proposal_generator.anchor_generator.strides = \
        list(torch_model.proposal_generator.anchor_generator.strides)
    torch_model.proposal_generator.anchor_generator.strides = \
        list(torch_model.proposal_generator.anchor_generator.strides)
    torch_model.roi_heads.in_features = torch_model.roi_heads.box_in_features = \
        list(torch_model.roi_heads.box_in_features)
    torch_model.roi_heads.mask_in_features = \
        list(torch_model.roi_heads.mask_in_features)
    DetectionCheckpointer(torch_model).resume_or_load(args.model)
    torch_model.eval()
