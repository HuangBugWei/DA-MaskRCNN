...
def do_testing(cfg, model, iter, dataloader, evaluator, dataset_name):
    lastIter = 0
    try:
        with open(os.path.join(cfg.train.output_dir, f"{dataset_name}-{cfg.model.roi_heads.box_predictor.test_score_thresh}ap.json"), "r") as f:
            lines = f.readlines()
    ...
    ret["iter"] = iter
    with open(os.path.join(cfg.train.output_dir, f"{dataset_name}-{cfg.model.roi_heads.box_predictor.test_score_thresh}ap.json"), "a") as f:
        json.dump(ret, f)
        f.write("\n")
