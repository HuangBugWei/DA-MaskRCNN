...
def single_image_process(image, model, threshold=0.5):
    ...
    with torch.no_grad():
        ...
        postprocess(predictions, scale_h, scale_v, og_h, og_w, threshold)
...
def main(args):
    ...
    if os.path.isfile(args.input):
        ...
        if im is not None:
            ...
            for i in range(100):
                start = time.time()
                predictions = single_image_process(im, model, args.thres)
